...
```bash
nexus-sdd spec "My first feature"
nexus-sdd orchestrate HDU-01 --phase apply
...
Nexus-SDD is the **control layer**:
...
```bash
git clone https://github.com/JesusDevs/nexus-sdd.git && cd nexus-sdd
./install.sh
nexus-sdd spec "My feature"
nexus-sdd orchestrate HDU-01 --status
```
...
├── nexus_sdd/              # Minimal Python CLI (typer + rich only)
...
│   ├── .nexus/config.yaml  # Project harness config (sensors/guides/steering)
...
nexus-sdd orchestrate HDU-NNN --phase apply
...
| Command | Description |
|---------|-------------|
| `nexus-sdd spec "Title"` | Create OpenSpec specification |
| `nexus-sdd orchestrate <HDU> --status` | Show phase progress + swarm mode |
| `nexus-sdd orchestrate <HDU> --phase <phase>` | Execute phase with right agent |
| `nexus-sdd orchestrate <HDU> --agent <name>` | Invoke specific persona |
| `nexus-sdd save --hdu-id <HDU>` | Save HDU to mnemo memory |
| `nexus-sdd status` | HDUs + harness health + sensor status |
| `nexus-sdd release v0.1.0 --all` | Semantic release with checks |
...
```bash
nexus-sdd status  # Shows sensor/guide health
```
...
harness:
  feedforward:
    skills_dir: skills/
    agent_profiles_dir: .nexus/profiles/
  feedback:
    sensors:
