...
When the user runs `nexus-sdd release v0.1.0 --project nexus-mnemo`
...
When the user runs `nexus-sdd release v0.1.0 --dry-run`
...
And invokes `nexus-sdd orchestrate HDU-06 --phase apply`
...
When the user runs `nexus-sdd release v0.1.0`
...
When `nexus-sdd release` generates the changelog
...
```gherkin
Given a monorepo with nexus-mnemo (Go) and nexus-sdd (markdown)
When the user runs `nexus-sdd release v0.2.0 --all`
