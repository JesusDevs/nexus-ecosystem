# Design: Rename engram-vec to gingx-mnemo
...
9. `gingx-sdd/install.sh`: references to binary and directory
...
2. **`mnemo` standalone without namespace** — Loses connection to Gingx-SDD ecosystem. The `gingx-` prefix indicates belonging.
...
## Decision
Full independence. `gingx-mnemo` as project name, `mnemo` as binary, `~/.mnemo/mnemo.db` as DB. No legacy columns, no env vars referencing Engram. Clean break.
