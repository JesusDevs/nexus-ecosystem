# Spec: Rename engram-vec to nexus-mnemo

## BDD Scenarios

### Scenario 1: mnemo binary functional
```gherkin
Given the project was renamed from engram-vec to nexus-mnemo
And the binary was compiled
When mnemo version is run
Then the output shows "nexus-mnemo v0.2.0"
...
Then the server responds with name="nexus-mnemo" and version="0.2.0"
And does not mention "engram-vec" in the identity
```

### Scenario 3: Standalone DB
```gherkin
Given nexus-mnemo is installed fresh
...
```gherkin
Given nexus-mnemo is installed via install.sh
