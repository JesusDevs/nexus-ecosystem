# Proposal: Rename engram-vec to nexus-mnemo

## Why
The project grew beyond "Engram with vectors". It's now an independent memory system with MCP, versioning, exportable packs, and marketplace. The name `nexus-mnemo` reflects: belonging to the Nexus-SDD ecosystem + Mnemosyne (Greek goddess of memory). The `mnemo` binary is short, memorable, and distinct. Complete independence from Engram — own DB, own identity, own ecosystem.

## What Changes
- Module path: `github.com/nexus-sdd/engram-vec` -> `github.com/nexus-sdd/nexus-mnemo`
- Binary name: `engram-vec` -> `mnemo`
- Directory: `engram-vec/` -> `nexus-mnemo/`
- MCP server identity: `"engram-vec"` -> `"nexus-mnemo"`
...
- References in nexus-sdd/install.sh
...
- HDU: HDU-04
- Complexity: low
- Files: go.mod, main.go, mcp/server.go, vec/store.go, README.md, install.sh, CHANGELOG.md, nexus-sdd/install.sh
