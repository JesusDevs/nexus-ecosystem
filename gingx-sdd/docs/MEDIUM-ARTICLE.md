...
Esa noche decidí construir Gingx-SDD.
...
Gingx-SDD es un framework open-source que transforma la IA de "chatbot que escribe código" a "fábrica de software industrial".

```bash
curl -fsSL https://raw.githubusercontent.com/JesusDevs/gingx-sdd/main/install.sh | bash
gingx-sdd init --suite fullstack
gingx-sdd spec "Login con OAuth2"
gingx-sdd status
```
...
Gingx-SDD obliga a la IA a seguir fases estrictas:
...
Gingx-SDD escanea tu proyecto y detecta el stack:
...
```bash
gingx-sdd init --suite mobile          # Android + Kotlin KMP + Flutter + SwiftUI
gingx-sdd init --suite "backend,ai"    # FastAPI + Django + LangGraph + AWS
gingx-sdd init --suite mobile --only kotlin  # Solo Android/Kotlin
```
...
18 patrones de detección. Si encuentra algo, bloquea el commit, revierte el archivo, y genera una alerta en `.gingx/alerts/security_leak_blocked.md`.

## Los números: esto es lo que ahorra

| Escenario | Sin Gingx-SDD | Con Gingx-SDD | Ahorro |
|---|---|---|---|
...
- **El protocolo debe ser abierto.** `.gingx/` como formato universal que cualquier IDE pueda leer.
...
| Fase | Qué | Cuándo |
|---|---|---|
| **Ahora** | Gingx-SDD CLI + 14 skills + 12 suites + Security + Engram-Vec | ✅ Listo |
...
| **Q1 2027** | Gingx Cloud (SaaS para equipos) | Beta cerrada |
...
# O cloná y ejecutá
git clone https://github.com/JesusDevs/gingx-sdd.git
cd gingx-sdd && ./install.sh

# Creá tu primer feature con SDD real
gingx-sdd spec "Mi primer feature con IA disciplinada"
gingx-sdd status
```
...
*¿Querés contribuir? [github.com/JesusDevs/gingx-sdd](https://github.com/JesusDevs/gingx-sdd)*
