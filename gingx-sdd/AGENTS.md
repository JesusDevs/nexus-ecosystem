# Gingx-SDD Agent Instructions

## Your Role
AI coding agent within the Gingx-SDD framework. Follow SDD methodology:
...
Gingx-SDD provides 7 specialized sub-agent personas in `skills/team/`:
...
├── templates/            # .gingx/ templates for new projects
...
3. **Security.** No hardcoded secrets. `gingx-sdd security` scans on every commit.
...
2. Register in `gingx_sdd/orchestrate.py` → `PHASE_AGENT_MAP` and `AGENT_ROLES`
3. Add to `cli.py` agent_map
4. Test: `gingx-sdd orchestrate <HDU> --agent <role>`

## Technology Stack (for this project)

- **Python 3.11+** — LangGraph harness, CLI (Typer), detector
- **Go** — gingx-mnemo (vector memory, MCP server)
