"""
Profile Loader — loads, validates, and lists team profiles from .gingx/profiles/
"""
...
def load_profile(name: str, profiles_dir: Path | None = None) -> Profile | None:
    """Load a profile by name from .gingx/profiles/<name>.profile.yaml or .yaml"""
...
def set_active_profile(name: str, gingx_dir: Path | None = None) -> bool:
    """Write active profile name to .gingx/current_profile.yaml."""
    if gingx_dir is None:
        gingx_dir = Path.cwd() / ".gingx"

    gingx_dir.mkdir(parents=True, exist_ok=True)
    profile_file = gingx_dir / "current_profile.yaml"
    profile_file.write_text(f"active_profile: {name}\n")
    return True


def get_active_profile(gingx_dir: Path | None = None) -> str:
    """Read active profile from .gingx/current_profile.yaml."""
    if gingx_dir is None:
        gingx_dir = Path.cwd() / ".gingx"
    pf = gingx_dir / "current_profile.yaml"
    if pf.exists():
        text = pf.read_text()
...
def _default_profiles_dir() -> Path:
    return Path.cwd() / ".gingx" / "profiles"
