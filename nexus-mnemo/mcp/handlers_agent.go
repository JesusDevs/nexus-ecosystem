...
// Calls the nexus-sdd Python CLI to assemble the prompt (shared engine).
func (s *Server) handleAgentDispatch(args map[string]interface{}) (string, error) {
	...
	// Find project root by looking for .nexus/
	projectRoot := findProjectRoot()

	// Build nexus-sdd CLI command
	cmdArgs := []string{
		"-m", "nexus_sdd", "team", "spawn", agent,
		"--task", task,
		"--profile", profile,
		"--mode", "prompt",
	}
	...
	cmd.Env = append(cmd.Env,
		"PYTHONPATH="+filepath.Join(projectRoot, "nexus-sdd"),
	)
...
// findProjectRoot locates the project root by searching for .nexus/ directory.
func findProjectRoot() string {
	...
	for {
		nexusDir := filepath.Join(abs, ".nexus")
		if stat, err := exec.Command("test", "-d", nexusDir).CombinedOutput(); err == nil && stat == nil {
			return abs
		}
